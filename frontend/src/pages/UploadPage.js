...
export default function UploadPage() {
  ...
  const onDrop = useCallback((accepted, rejected) => {
    ...
    if (rejected.length > 0) {
      ...
      if (reason.code === 'file-too-large') setError('File is too large. Maximum size is 5 MB.');
  ...
  const handleUploadAndAnalyze = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Analysis failed. You can retry from the dashboard.');
      setStage('idle');
      navigate('/dashboard');
    }
  };
  ...
  const currentStep = stage === 'idle' ? 0 : stage === 'uploading' ? 1 : stage === 'analyzing' ? 2 : 3;

  const dropzoneStyle = {
    border: `2px dashed ${isDragReject ? 'var(--danger)' : isDragActive ? 'var(--primary)' : file ? 'var(--accent)' : 'var(--border-light)'}`,
    borderRadius: 'var(--radius-xl)',
    background: isDragActive ? 'rgba(108,99,255,0.06)' : file ? 'rgba(0,212,170,0.04)' : 'var(--bg-elevated)',
    padding: '52px 32px',
    ...
    <div style={{ maxWidth: 700, margin: '0 auto', animation: 'fadeIn 0.4s ease' }}>
          ...
          <React.Fragment key={step.label}>
            <div style={{ flex: 1, textAlign: 'center' }}>
              <div style={{
                ...
                fontSize: '1.2rem',
                background: i < currentStep ? 'var(--accent)' : i === currentStep ? 'var(--primary)' : 'var(--bg-elevated)',
                border: `2px solid ${i < currentStep ? 'var(--accent)' : i === currentStep ? 'var(--primary)' : 'var(--border)'}`,
                transition: 'all var(--transition-slow)',
                boxShadow: i === currentStep ? '0 0 16px var(--primary-glow)' : 'none',
              }}>
                {i < currentStep ? '✓' : step.icon}
              </div>
              <div style={{ fontSize: '0.72rem', fontWeight: 600, color: i <= currentStep ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                {step.label}
              </div>
              <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginTop: 2, display: window.innerWidth > 500 ? 'block' : 'none' }}>
                {step.desc}
              </div>
            </div>
            {i < steps.length - 1 && (
          ...
          {stage === 'uploading' ? (
            <div>
              ...
              <div style={{ maxWidth: 300, margin: '0 auto' }}>
                <ProgressBar value={uploadProgress} color="var(--primary)" animated />
              </div>
            </div>
          ) : stage === 'analyzing' ? (
            <div>
            ...
            <div>
              ...
              <h3 style={{ color: 'var(--success)' }}>Done! Redirecting to results…</h3>
            </div>
          ) : file ? (
            <div>
              <div style={{
              ...
              <h3 style={{ marginBottom: 6, color: 'var(--accent)' }}>File Selected!</h3>
            ...
            <div>
              ...
              <h3 style={{ marginBottom: 8 }}>
                {isDragReject ? 'Invalid file type' : isDragActive ? 'Drop it here!' : 'Drag & drop your resume'}
              </h3>
              <p style={{ color: 'var(--text-muted)', marginBottom: 20, fontSize: '0.9rem' }}>
                {isDragReject ? 'Only PDF files are accepted' : 'or click to browse files'}
              </p>
              <div style={{
                display: 'inline-flex', alignItems: 'center', gap: 8,
                padding: '10px 20px', background: 'var(--primary-glow)',
      ...
      {error && (
        <div style={{
          ...
          color: 'var(--danger)', fontSize: '0.875rem', marginBottom: 20,
      ...
      {/* ── Action Button ─────────────────────────────────────────── */}
      <Button
        fullWidth size="lg" loading={isProcessing}
        disabled={!file || isProcessing}
        onClick={handleUploadAndAnalyze}
      >
        {isProcessing
          ? stage === 'uploading' ? `Uploading… ${uploadProgress}%` : 'Analyzing with AI…'
          : '⚡ Upload & Analyze Now'}
      </Button>

      {/* ── Tips ──────────────────────────────────────────────────── */}
      <Card style={{ marginTop: 24, background: 'transparent', border: '1px solid var(--border)' }}>
        <h4 style={{ marginBottom: 12, fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
          For best results:
        </h4>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            ...
            <p key={tip} style={{ fontSize: '0.83rem', color: 'var(--text-muted)' }}>{tip}</p>
          ))}
        </div>
      </Card>
    </div>
