...
import {
  Card, Button, Badge, ScoreRing, ProgressBar, TagCloud, SectionHeader, Spinner,
} from '../components/UI';
import toast from 'react-hot-toast';

const Section = ({ title, icon, children, style }) => (
  <Card style={{ ...style }}>
    <h4 style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, color: 'var(--text-primary)' }}>
...
const ListItems = ({ items = [], variant = 'default', emptyMsg }) => {
  if (!items.length) return <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{emptyMsg}</p>;
  return (
      ...
      {items.map((item, i) => (
        <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          <span style={{
            ...
            background: variant === 'success' ? 'rgba(0,212,170,0.15)' : variant === 'danger' ? 'rgba(255,107,107,0.15)' : 'rgba(108,99,255,0.15)',
            color: variant === 'success' ? 'var(--success)' : variant === 'danger' ? 'var(--danger)' : 'var(--primary-light)',
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (resumeData.status === 'fulfilled') setResume(resumeData.value.resume);
        if (analysisData.status === 'fulfilled') setAnalysis(analysisData.value.analysis);
  ...
  if (error) {
    return (
      <div style={{ textAlign: 'center', padding: 48 }}>
        ...
        <Button onClick={() => navigate('/dashboard')} variant="secondary" style={{ marginTop: 16 }}>
          ← Back to Dashboard
        </Button>
      </div>
    );
  ...
  if (!analysis) {
    return (
      <div style={{ maxWidth: 500, margin: '80px auto', textAlign: 'center', animation: 'fadeIn 0.4s ease' }}>
        <div style={{ fontSize: '3.5rem', marginBottom: 20 }}>🤖</div>
        <h2 style={{ marginBottom: 8 }}>No Analysis Yet</h2>
        <p style={{ color: 'var(--text-muted)', marginBottom: 28 }}>
          {resume ? `"${resume.originalName}" hasn't been analyzed yet.` : 'This resume hasn\'t been analyzed yet.'}
  ...
  const { overallScore, scoreBreakdown, skillsFound, missingKeywords, strengths, weaknesses,
    suggestions, overallFeedback, detectedJobTitle, experienceLevel, industryMatch,
    processingTimeMs, aiModel, grade } = analysis;
  ...
  return (
    <div style={{ animation: 'fadeIn 0.4s ease' }}>
      ...
      <SectionHeader
        ...
        subtitle={resume ? `Analysis for: ${resume.originalName}` : 'AI-Powered Resume Report'}
        action={
      ...
      {/* ── Overall Score Hero ────────────────────────────────── */}
      <Card glow style={{ marginBottom: 20, background: 'linear-gradient(135deg, var(--bg-card), var(--bg-elevated))' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 32, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
              <h2 style={{ margin: 0 }}>
                {overallScore >= 80 ? 'Excellent Resume!' : overallScore >= 60 ? 'Good Resume' : overallScore >= 40 ? 'Needs Improvement' : 'Major Revisions Needed'}
              </h2>
            ...
            <div style={{ marginTop: 14, display: 'flex', gap: 16, fontSize: '0.75rem', color: 'var(--text-muted)' }}>
              ...
              {processingTimeMs > 0 && <span>Time: {(processingTimeMs / 1000).toFixed(1)}s</span>}
            </div>
          </div>
        </div>
      </Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20, marginBottom: 20 }}>
        <Section title="Score Breakdown" icon="📊">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              { label: 'Formatting & Structure', key: 'formatting', icon: '📐' },
        ...
        <Section title="Resume Overview" icon="📋">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { label: 'Overall Score', value: `${overallScore}/100`, color: scoreColor },
              ...
              { label: 'Skills Detected', value: `${skillsFound?.length || 0} skills` },
              { label: 'Missing Keywords', value: `${missingKeywords?.length || 0} items` },
              resume && { label: 'Word Count', value: `${resume.wordCount} words` },
              resume && { label: 'Pages', value: `${resume.pageCount} page${resume.pageCount > 1 ? 's' : ''}` },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20, marginBottom: 20 }}>
        <Section title="Skills Found" icon="✅">
          {skillsFound?.length > 0
            ...
            : <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No skills detected. Add a dedicated Skills section.</p>
          }
        </Section>
        ...
        <Section title="Missing Keywords" icon="❌">
          {missingKeywords?.length > 0
            ...
            : <p style={{ color: 'var(--success)', fontSize: '0.875rem' }}>Great! No critical keywords missing.</p>
          }
          {missingKeywords?.length > 0 && (
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: 12 }}>
              Consider adding these to improve ATS compatibility and keyword matching.
            </p>
          )}
        </Section>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20, marginBottom: 20 }}>
        <Section title="Strengths" icon="💪" style={{ borderTop: `3px solid var(--success)` }}>
          <ListItems items={strengths} variant="success" emptyMsg="Run the analysis again for detailed strengths." />
        </Section>

        <Section title="Weaknesses" icon="⚠️" style={{ borderTop: `3px solid var(--danger)` }}>
          <ListItems items={weaknesses} variant="danger" emptyMsg="No major weaknesses detected!" />
        </Section>
      </div>

      {/* ── Suggestions ──────────────────────────────────────────── */}
      <Section title="Improvement Suggestions" icon="💡" style={{ marginBottom: 20, borderTop: `3px solid var(--primary)` }}>
            ...
            {suggestions.map((s, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  <strong style={{ color: 'var(--primary-light)' }}>#{i + 1}</strong> {s}
                </span>
              </div>
            ))}
          </div>
        ) : (
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No specific suggestions — your resume looks great!</p>
        )}
      </Section>

      {/* ── CTA Banner ───────────────────────────────────────────── */}
      <Card style={{ textAlign: 'center', background: 'linear-gradient(135deg, rgba(108,99,255,0.08), rgba(0,212,170,0.08))', border: '1px solid rgba(108,99,255,0.2)' }}>
        <h3 style={{ marginBottom: 8 }}>Ready to improve your resume?</h3>
        <p style={{ color: 'var(--text-muted)', marginBottom: 20, fontSize: '0.9rem' }}>
          Make the changes above and upload your updated resume to see your score improve.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Button onClick={() => navigate('/upload')}>↑ Upload Updated Resume</Button>
