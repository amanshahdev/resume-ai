...
import { apiGetDashboardStats, apiGetResumes, apiRunAnalysis, apiDeleteResume } from '../utils/api';
import {
  Card, Button, Badge, StatCard, ProgressBar,
...
export default function DashboardPage() {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.4s ease' }}>
      {/* ── Welcome Banner ─────────────────────────────────────────── */}
      <div style={{
        background: 'linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 40%, #1a1060 100%)',
        ...
        {loadingStats ? (
          ...
          <>
            <StatCard
              ...
              icon={<IconSVG d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z M14 2v6h6" size={20} />}
              color="var(--primary)"
            />
            <StatCard
              label="Analyses Run"
              value={stats?.totalAnalyses ?? 0}
              icon={<IconSVG d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" size={20} />}
              color="var(--accent)"
            />
            <StatCard
              label="Average Score"
              value={stats?.averageScore ? `${stats.averageScore}` : '—'}
              icon={<IconSVG d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" size={20} />}
              color="var(--warning)"
            />
            <StatCard
              label="Best Score"
              value={stats?.highestScore ? `${stats.highestScore}` : '—'}
              icon={<IconSVG d="M8.21 13.89L7 23l5-3 5 3-1.21-9.12 M15 7a3 3 0 11-6 0 3 3 0 016 0z" size={20} />}
              color="var(--success)"
            />
          </>
        )}
            ...
            <Card key={resume._id} hover style={{ padding: '16px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                ...
                {resume.analysis && (
                  <div style={{ textAlign: 'center', flexShrink: 0 }}>
                    <div style={{
                      ...
                      fontSize: '1.4rem', color: resume.analysis.overallScore >= 70 ? 'var(--success)' : resume.analysis.overallScore >= 50 ? 'var(--warning)' : 'var(--danger)',
                    }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                  {resume.status === 'analyzed' && (
                    <Button size="sm" variant="secondary" onClick={() => navigate(`/results/${resume._id}`)}>
                      View Results
                    </Button>
                  )}
                  {(resume.status === 'uploaded' || resume.status === 'failed') && (
                    <Button size="sm" loading={analyzingId === resume._id}
              ...
              {resume.analysis && (
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
                  <ProgressBar value={resume.analysis.overallScore} max={100} height={5} />
                </div>
              )}
            </Card>
          ))}
      ...
      {/* ── Quick Tips ────────────────────────────────────────────── */}
      {resumes.length > 0 && (
        <Card style={{ marginTop: 28, background: 'linear-gradient(135deg, rgba(0,212,170,0.05), rgba(108,99,255,0.05))', border: '1px solid rgba(0,212,170,0.2)' }}>
          <h4 style={{ marginBottom: 12, color: 'var(--accent)' }}>💡 Pro Tips</h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
            {[
              { icon: '🎯', tip: 'Target each resume to the specific job description' },
              { icon: '📊', tip: 'Quantify achievements with numbers and percentages' },
              { icon: '🔑', tip: 'Use keywords from the job posting in your resume' },
              { icon: '✂️', tip: 'Keep it to 1 page for under 10 years experience' },
            ].map(({ icon, tip }) => (
